...
const service = services.find(
  (service) => service.slug === route.params.service
);
    ...
    <main v-if="service">
      ...
      >
        ...
        <div class="text-center flex-center gap text-sm">
          ...
          <Icon
            ...
            class="text-mute"
          />
          <p class="cursor-context-menu font-semibold text-mute">
            {{ service.title }}
          </p>
        </div>
      </header>
      ...
      <div class="px-4 py-10 md:px-40 lg:px-60 xl:px-80">
        <div>
          ...
          <p class="mt-4 text-lg md:text-center text-mute">
            {{ service.overview }}
          </p>

          <div class="mt-6 grid gap-10">
            <section
              v-for="desc in service.desc"
              :key="desc.title"
              :id="desc.title.toLowerCase()"
            >
              <header class="text-lg text-mute font-semibold">
                {{ desc.title }}
              </header>
