...
const {
  ...
  data,
  mutate
} = useSWRV<InvestmentPlansGetApiResponse>("/api/admins/me/investment-plans", $fetch)
            ...
            <VCard v-for="plan in data?.investmentPlans" :key="plan.id" :header="plan.name">
              <div class="grid">
                ...
                <div class="mt-2 flex items-center gap-1 justify-center flex-wrap">
                  <p v-for="(tier, index) in plan.tiers" :key="tier.name" class="text-sm flex items-center gap-1">
                ...
                <div class="mt-2 grid grid-cols-2 gap-2 w-full">
                  <Button @click="
                    ...
                    " label="Edit" icon="pi pi-file-edit" size="small" fluid />
