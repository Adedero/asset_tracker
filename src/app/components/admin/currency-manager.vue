...
const updatedCurrency = ref<Partial<Currency>>(currency ? JSON.parse(JSON.stringify(currency)) : {});
...
const manageCurrency = async () => {
  if (currency) {
    ...
    updatedCurrency.value = Object.fromEntries(Object.entries(updatedCurrency.value).filter(([, v]) => v !== null));
...
<template>
  <div>
    ...
    <Dialog v-model:visible="visible" modal :header="currency ? 'Edit Currency' : 'New Currency'" class="w-80 md:w-96">
      <div class="grid gap-4">
        <div class="flex flex-col items-center justify-center gap-1">
          ...
          <VFileUploader size="small" accept="image/*" :max-file-size="MAX_CURRENCY_IMG_SIZE"
            @select="(files: IFile[]) => (updatedCurrency.image = files[0].dataUrl ?? '')" @upload="manageCurrency"
            @cancel="updatedCurrency.image = currency?.image ?? ''" class="w-full" :loading="isCreating || isUpdating"
        ...
        <div class="grid">
          ...
          <ToggleButton v-model="updatedCurrency.isAvailableForWithdrawal" onLabel="Yes" offLabel="No" />
        </div>
        ...
        <div v-if="updatedCurrency.isAvailableForWithdrawal" class="grid">
          ...
          <InputNumber v-model="updatedCurrency.withdrawalCharge" prefix="$" :max-fraction-digits="2" fluid />
        </div>

        <div class="grid mt-3">
          <Button 
            @click="manageCurrency" :loading="isCreating || isUpdating" 
            :disabled="isCreating || isUpdating || disabled" 
            label="Submit" 
            fluid 
          />
