...
export interface DepositGiftCardInitApiResponse extends ApiResponse {
  total: number;
  rates: ExchangeRateData<["USD", "CAD", "GBP"]>;
}
...
export default api(
  ...
  defineHandler(async (req) => {
    const cards = req.validatedBody as z.infer<typeof Schema>;

    const rates: ExchangeRateData<["USD", "CAD", "GBP"]> = {
      USD: 1,
      CAD: 0,
      GBP: 0
    };
    ...
    const payload: DepositGiftCardInitApiResponse = {
      success: true,
