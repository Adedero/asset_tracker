...
export async function onTransactionStatusUpdate({ user, transaction }: TransactionAlertData) {
  ...
  const message = (name?: string) => ({
    ...
    info: conditional(
      `The ${transaction.transactionType} request with transaction ID ${transaction.id} has been approved.`.concat(
        conditional(
          `Your wallet has been credited with $${transaction.amountInUSD.toLocaleString()}.`,
          "",
      ...
      "The "
        ...
        .concat(
          conditional(
            `$${transaction.actualAmountInUSD.toLocaleString()} has been refunded to your wallet.`,
            "",
            transaction.transactionType === "WITHDRAWAL" && !!name
          )
        )
  ...
  function conditional(
    str1: string,
    str2: string,
    condition: boolean = transaction.transactionStatus === "SUCCESSFUL"
  ) {
